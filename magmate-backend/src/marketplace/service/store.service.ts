...
export class StoreService {
  ...
  async create(dto: CreateMagasinDto) {
    ...
    const user = await this.userRepository.findOne({ where: { id: dto.proprietaireId } });
    if (!user) {
    ...
    try {
      ...
      throw new InternalServerErrorException("Erreur interne lors de la création du magasin");
    }
  }
  ...
  async findAll() {
    try {
      ...
      throw new InternalServerErrorException("Erreur lors de la récupération des magasins");
    }
  }
  ...
  async findOne(idMagasin: number) {
    try {
      ...
      throw new InternalServerErrorException(`Erreur lors de la récupération du magasin avec l'ID ${idMagasin}`);
    }
  }
  ...
  async update(idMagasin: number, dto: UpdateMagasinDto) {
    ...
    try {
      ...
      throw new InternalServerErrorException('Erreur lors de la mise à jour du magasin');
    }
  }
  ...
  async remove(idMagasin: number) {
    ...
    try {
      ...
      throw new InternalServerErrorException('Erreur lors de la suppression du magasin');
    }
  }
}
